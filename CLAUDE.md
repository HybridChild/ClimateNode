...
- **`docs/` — terse project references**: decisions, rationale, and verified facts. `docs/README.md` lists the pairings.
- Pairings: `communication-guide` ↔ `mqtt-design`, `network-stack-guide` ↔ `network-bringup`, `can-guide` ↔ `can-bringup`, `zephyr-build-system-guide` ↔ `build-system-overview`, `sensor-api-guide` ↔ `sensor-bringup`, `testing-guide` ↔ `test-strategy`. `protobuf-guide.md`'s reference half is `proto/node.proto`, which carries its decisions inline; `zbus-guide.md`'s is the header comments in `shared/app_channels.h` and `gateway/src/relay.h`. `language-cpp.md` and `shell-guide.md` have no reference half.
- **Exception:** `docs/firmware-mqtt-walkthrough.md` teaches, but it is a guided reading of *this repo's* `gateway/src/main.cpp` rather than a general concept, so it lives with the references — keep it in sync when the client changes.
