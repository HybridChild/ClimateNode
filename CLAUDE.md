...
- **All five phases work end to end, verified on hardware 2026-07-21.** `firmware/` is a working Zephyr **C++** app (see `notes/language-cpp.md`) split across two threads: `sensor.cpp` reads the SCD-40 over I²C via the sensor API and publishes to a **zbus** channel; `main.cpp` brings up IPv4 on the on-board Ethernet and runs an MQTT client that publishes nanopb-encoded `Telemetry` and answers `Command`s with `Ack`s. They meet on the channels in `firmware/src/app_channels.h` — read its header comment before touching either side. `proto/` holds the schema; `host/` holds the Python paho harness (`monitor.py`, `command.py`) — run it **on the Pi**, which is the only machine that can reach the broker. Verified against the bench: telemetry decode, all three commands, out-of-range rejection, duplicate suppression, a malformed payload that did not desync the stream, and a 66 s broker outage the sensor thread sampled straight through. What remains is the **schema-versioning exercise** (add a field, decode old↔new).
...
- **No test/lint tooling exists.** Verification is build → flash → observe on hardware (console, `net` shell commands, or the Pi). The nearest thing to a test suite is the **lab at the end of each guide** — `communication-guide.md` §9, `zbus-guide.md` §10, `protobuf-guide.md` §10, `sensor-api-guide.md` §11, `zephyr-build-system-guide.md` §11, `language-cpp.md` §12, plus *Testing the Last Will* in `mqtt-design.md` and *Bring-up checks* in `sensor-bringup.md`. **Write bench results as a procedure someone can re-run — commands, expected output, and what it proves — never as a dated report of a past run.** When behaviour changes, update the lab that covers it.
