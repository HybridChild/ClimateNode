...
**Still to do:**
- **A second node over CAN** — a Nucleo-F072RB with a BME280 as a peer node with its own MQTT identity, relayed by the H753ZI acting as a *gateway* that forwards its already-encoded Protobuf without decoding it. Concepts in [`notes/can-guide.md`](notes/can-guide.md), what is actually built in [`docs/can-bringup.md`](docs/can-bringup.md). **Working end to end on hardware.** A terminated 500 kbit/s two-node bus carries a 1 Hz heartbeat and ISO-TP in both directions: the peer's BME280 telemetry reaches `node/2/telemetry`, and a command published to `node/2/command` is answered by the peer's own `DeviceInfo` on `node/2/ack`. The gateway holds **two MQTT connections**, `nucleo-1` and `nucleo-2`, because MQTT 3.1.1 allows one Last Will per connection and the peer's status topic needs its own — so a dying gateway marks both nodes offline through the broker, with no firmware in the path. Because the two sensors measure different things, the pair also demonstrates what `optional` measurements are for: node 1 reports CO₂ and no pressure, node 2 the reverse, and each absent measurement decodes as absent rather than as a zero.
