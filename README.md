...
- **Peer node (phase 6):** ST Nucleo-F072RB (STM32F072RB, Cortex-M0) with an Adafruit BME280 — a *second* node with its own MQTT identity, reached over CAN through the H753ZI acting as a gateway. 128 KB flash, **16 KB RAM**, no FPU, and classic CAN 2.0B only (bxCAN, no CAN-FD), which is the whole point: eight payload bytes per frame is what forces real segmentation.
...
**Still to do:**
- **A second node over CAN** — a Nucleo-F072RB with a BME280 as a peer node with its own MQTT identity, relayed by the H753ZI acting as a *gateway* that forwards its already-encoded Protobuf without decoding it. Concepts in [`notes/can-guide.md`](notes/can-guide.md), what is actually built in [`docs/can-bringup.md`](docs/can-bringup.md). Done so far: the CAN controller proven in internal loopback on the gateway; the shared link contract with its tests; the whole peer-node firmware (heartbeat, ISO-TP both ways, the BME280); and the gateway's relay, which publishes the peer's payloads to `node/2/{telemetry,ack,status}` and routes `node/2/command` back down without ever decoding them. The gateway now holds **two MQTT connections**, `nucleo-1` and `nucleo-2`, because MQTT 3.1.1 allows one Last Will per connection and the peer's status topic needs its own. Not done: any frame at all on a real bus, since the transceivers are not bought.
...
- `firmware/` — the **H753ZI gateway**: sensor node *and* CAN-to-MQTT relay. A Zephyr application in **C++17** (`prj.conf` with `CONFIG_CPP=y`, `CMakeLists.txt`, `src/*.cpp`, and a **board overlay** defining the I²C bus + `sensirion,scd40` node). Generates `node.pb.c/.h` at build time so it can't drift from the schema.
...
| zbus (the internal bus) | [`zbus-guide.md`](notes/zbus-guide.md) | [`firmware/src/app_channels.h`](firmware/src/app_channels.h) and [`relay.h`](firmware/src/relay.h) (decisions in the header comments) |
