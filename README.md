...
**Goal:** rehearse the *core daily interface* of the Vitrolife role — framed Protobuf messages over TCP between a Zephyr firmware node and a host PC — and get fluent in the parts that bite in production (framing, versioning, nanopb on a constrained target). See `../../Vitrolife/role.md` and `../../Vitrolife/CLAUDE.md` for why this is the Tier-1 prep focus.

This is a **prep/practice project**, not the real product. The aim is learning the patterns, not shipping.

To make the telemetry *real* (rather than a hard-coded counter), the node reads a live sensor — an Adafruit SCD-40 CO₂/temperature/humidity breakout — over I²C and streams those readings out over Ethernet. This mirrors the real product shape: a sensor-bearing node marshalling readings into Protobuf for the host PC.
...
- **Host:** Raspberry Pi 5 (Linux) — native Gigabit Ethernet, wired **direct-cable** to the Nucleo (no switch). Static IPs on both ends in one subnet, e.g. Pi `192.168.10.1` / Nucleo `192.168.10.2`, mask `255.255.255.0`, no gateway. The Nucleo's LAN8742 PHY has Auto-MDIX, so a normal straight-through cable works. Runs the Python test harness; can stay permanently wired as a dedicated bench host.

## What "done" looks like (scope)
A Zephyr app on the Nucleo that:
1. Brings up the network interface and listens on a TCP port.
...
3. Pushes those readings as a **length-prefixed Protobuf telemetry message** (~every 5 s), and answers a **length-prefixed Protobuf command/ack** pair — encoding/decoding with **nanopb**.
4. Talks to a **host-side test harness** on the Raspberry Pi (Python) that decodes and logs the telemetry stream and can send commands. *(Optional higher-fidelity pass: re-run the harness in C#/.NET on a Windows box later to mirror the real Windows app side.)*

## The three things to actually learn (don't skip these)
1. **Message framing over a TCP stream** — Protobuf is *not* self-delimiting. Implement a length-prefix (e.g. 4-byte LE length header) and handle partial reads / reassembly correctly.
2. **Schema versioning & backward-compat** — exercise adding a field and talking old↔new: field numbers, `optional`, unknown-field handling. This is the firmware↔SW-team contract in miniature.
3. **nanopb on a constrained target** — `.proto` → generated C, `.options` files, fixed-size vs callback fields, no-malloc/static allocation.
...
The SCD-40's ~5 s sample rate defines the natural telemetry period; the command path lets the host change or force it.
...
- `host/` — host test harness (Python encode/decode + send/receive client; logs telemetry).
- `docs/` — notes: framing format decision, versioning experiments, sensor/overlay setup, gotchas.

## Open decisions (resolve before coding)
- Transport detail: raw TCP + custom length-prefix framing (default for this exercise). gRPC is out of scope — too heavy for the embedded side.
