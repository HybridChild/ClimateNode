...
The point of it is what it removes from the bench list rather than what it adds to the table. `tests/heartbeat/` can prove `heartbeat_pack()` produces eight correct bytes; it cannot prove those bytes ever reach a filter, that the identifiers `can_link.h` computes match anything, or that ISO-TP's segmentation carries a payload of the size this project actually sends. This suite answers all three off-target, which leaves the bench responsible for only what is genuinely electrical.
...
**One responsibility per translation unit, so that two of them link without hardware.** The gateway is `main.cpp` (the MQTT sessions), `sensor.cpp` (acquisition), `relay.cpp` (the CAN side), `protocol.cpp` (the wire format) and `commands.cpp` (command semantics). Collapse any of those into `main.cpp` and it stops being reachable from a test, because that file cannot be compiled without the network stack behind it; kept separate, `protocol.cpp` and `commands.cpp` depend on nanopb and zbus and nothing else. No seam was carved for the tests and no test-only parameter exists — each file is justifiable in a review without mentioning tests, which is the check on whether a split is design or damage. The split costs FLASH +72 B and no RAM. `protocol.cpp` and `commands.cpp` sit in `shared/` rather than under `gateway/` for a second reason on top: the peer node links the same two files.
...
**`CAN_MODE_LOOPBACK` is not implied by the loopback driver's name.** `drivers/can/can_loopback.c:79` drops every frame when the mode bit is clear — `can_send()` still returns 0, the transmit-done callback still fires, and no receive filter is consulted. Omit the mode and the suite does not error; it reports the two tests that assert something is *not* received as passes, and fails the rest. That is why both negative tests are mutation-checked (`../notes/testing-guide.md` §10, Exercise 4): a test that cannot fail is worse than no test, and a negative test in a silent environment is the easiest way to write one. `can_set_mode()` returns `-EBUSY` once the controller is started, so the mode is set before `can_start()`.
...
- **`main.cpp`'s MQTT session** — connect, keepalive, reconnect backoff, the poll loop and the deadlines it folds into one timeout. Needs a broker and a socket.
