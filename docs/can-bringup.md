...
**Commands sent during the outage are refused, not queued.** The host sees `ACK_STATUS_FAILED` with detail `no route to node over CAN` within about a second, and nothing is delivered when the bus returns: the relay has no store-and-forward, so the host holds the intent and owns the retry. Why refusing beats queueing, and what a firmware-update channel would need instead, is *Nothing is queued for later delivery* in [`mqtt-design.md`](mqtt-design.md).

**Not checked here, but checked elsewhere:** the Last Will on `node/2/status` when the *gateway* dies is the complementary failure and lives in [`mqtt-design.md`](mqtt-design.md) — it is a broker behaviour, not a CAN one, and confusing it with this step is easy. **Still undone anywhere:** duplicate suppression against the peer specifically, and a loopback check of the peer's own ISO-TP path, which remains possible.
