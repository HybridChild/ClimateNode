...
- A failed read is reported, not hidden: the reading still goes out carrying `SENSOR_READING_ERROR`. The host cannot distinguish a broken sensor from a dead node if we simply go quiet, which is why the status field exists.
