...
Ten teaching documents, each building one concept from first principles and using this repo — a CO₂ node on a Nucleo-H753ZI talking to a Raspberry Pi — as the running example. They are meant to be readable by someone who has never seen this project.
...
8. **[`zbus-guide.md`](zbus-guide.md)** — the one piece *inside* the firmware: the in-process bus where the sensor thread hands each reading to the MQTT thread. It lands best now, once you have seen both threads it joins.
...
| 8 | [`zbus-guide.md`](zbus-guide.md) | What is an in-process message bus, and why does a two-thread firmware want one? | threads and blocking calls; `poll()` and mutexes are explained in §1–§2 | [`app_channels.h`](../firmware/src/app_channels.h) and [`relay.h`](../firmware/src/relay.h) header comments |
